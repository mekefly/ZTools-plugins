...
class InitService {
    ...
    #init_recentProjects() {
        ...
        Object.keys(this.channels).forEach(displayName => {
            ...
            let recentProjectsFile = window.ztools.getPath("appData").replace("\ ", " ") + "/JetBrains/" + channel.dataDirectoryName + "/options/recentProjects.xml"
            ...
            for (let index = 0; index < xml_entry.length; index++) {
                ...
                if (window.ztools.isWindows() && entry.getAttribute("key").startsWith("~")) {
                    entry.setAttribute("key", entry.getAttribute("key").replace("~", window.ztools.getPath("home")))
                }
